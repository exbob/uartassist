                      ...
                      int *stop_bit)
{
	...
	if (str == NULL || data_bit == NULL || parity == NULL ||
	    stop_bit == NULL) {
		errno = EINVAL;
		return -1;
	}
...
void print_usage(const char *program_name)
{
	...
	printf("  -m, --mode <mode>          Working mode: loopback/send/recv (required)\n");
	printf("  -d, --device <device>       Serial port device (default: %s)\n",
	       DEFAULT_DEVICE);
	printf("  -b, --baud <baudrate>      Baud rate (default: %d)\n",
	       DEFAULT_BAUD);
	printf("  -c, --config <config>      UART config, format: databits parity stopbits (default: %d%c%d)\n",
	       DEFAULT_DATA_BIT, DEFAULT_PARITY, DEFAULT_STOP_BIT);
	printf("                            Examples: 8N1, 7E1, 8O2\n");
	printf("  -h, --help                 Show this help message\n");
	printf("\n");
	printf("Loopback Mode Options:\n");
	printf("  -s, --send <string>        Send string for loopback test (default: %s)\n",
	       DEFAULT_SEND_STRING);
	printf("  -f, --format <format>       Send format: ascii/hex (default: ascii)\n");
	printf("                            If hex, string is parsed as hex (e.g., af37126b4A = 5 bytes)\n");
	printf("\n");
	printf("Send Mode Options:\n");
	printf("  -s, --send <string>        Send string (default: %s)\n",
	       DEFAULT_SEND_STRING);
	printf("  -i, --interval <ms>        Send interval in milliseconds, 1-10000 (default: %d)\n",
	       DEFAULT_SEND_INTERVAL);
	printf("  -n, --count <count>        Send count, 0 means infinite (default: %d)\n",
	       DEFAULT_SEND_COUNT);
	printf("  -f, --format <format>       Send format: ascii/hex (default: ascii)\n");
	printf("                            If hex, string is parsed as hex (e.g., af37126b4A = 5 bytes)\n");
	printf("\n");
	printf("Receive Mode Options:\n");
	printf("  -f, --format <format>      Output format: ascii/hex (default: ascii)\n");
	printf("\n");
	printf("Examples:\n");
...
int parse_args(int argc, char *argv[], uart_config_t *config)
{
	...
	while ((opt = getopt_long(argc, argv, "d:b:c:m:s:i:n:f:h",
		...
		case 'c':
			if (parse_uart_config(optarg, &data_bit, &parity,
			                      &stop_bit) < 0) {
				return -1;
			}
		...
		case 'm':
			if (strcmp(optarg, "loopback") == 0) {
				...
				pr_error("Invalid mode: %s (should be loopback/send/recv)\n",
				         optarg);
				return -1;
			}
			mode_set = 1;
		...
		case 'i':
			...
			if (*endptr != '\0' || config->send_interval < 1 ||
				...
				pr_error("Invalid interval: %s (should be 1-10000)\n",
				         optarg);
				return -1;
			}
			break;
		...
		case 'f':
			if (strcmp(optarg, "ascii") == 0) {
				...
				pr_error("Invalid format: %s (should be ascii/hex)\n",
				         optarg);
				return -1;
			}
			break;
	...
	/* 检查必需参数 */
	if (!mode_set) {
		pr_error("Mode is required (-m loopback/send/recv)\n");
		print_usage(argv[0]);
		return -1;
	}
